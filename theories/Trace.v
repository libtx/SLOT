...
Section transition_system.
  ...
  Class TransitionSystem :=
    { future : State -> Future -> Prop;
      ...
      future_equiv : forall a b fut,
        a == b ->
        future a fut <-> future b fut;
    }.
...
Notation "A '~>[]'" := (A (fut_result I)) (at level 21) : slot_scope.
...
Section state_reachability.
  ...
  Lemma trace_equiv t s1 s1' s2 :
    Trace s1 s1' t ->
    s1 == s2 ->
    exists2 s2',
      Trace s2 s2' t & s1' == s2'.
  Proof.
    intros H1. generalize dependent s2.
    induction H1; intros.
    - exists s2; sauto.
    - apply future_equiv with (fut := fut_cont label s') in H0.
      apply H0 in H.
      exists s2'.
      + constructor.
...
Section definitional_commutativity.
  Context `{TS : TransitionSystem}.

  Definition labels_commute_defn (l1 l2 : Label) : Prop :=
    forall (s s' : State),
      Trace TS s s' [l1; l2] <-> Trace TS s s' [l2; l1].

  Global Instance events_commuteSymm : Symmetric labels_commute_defn.
  Proof.
    intros a b Hab s s''.
    unfold labels_commute_defn in *. specialize (Hab s s'').
    now symmetry in Hab.
  Qed.
End definitional_commutativity.

Section communativity.
  Context `{TS : TransitionSystem}.

  Definition equiv_commute_swap (l1 l2 : Label) (s s' : State) : Prop :=
    Trace _ s s' [l1; l2] ->
    exists2 s'',
      s' == s'' & Trace _ s s'' [l2; l1].

  Definition labels_commute (l1 l2 : Label) : Prop :=
    forall (s s' : State),
      equiv_commute_swap l1 l2 s s' <-> equiv_commute_swap l2 l1 s s'.

  Global Instance events_commute_equivSymm : Symmetric labels_commute.
  Proof.
    unfold labels_commute. sauto.
  Qed.
End communativity.
...
Section trace_properties.
  ...
  Lemma trace_commutative_permutation s1 s1' t1 t2 :
    Trace TS s1 s1' t1 ->
    RestrictedPermutation labels_commute t1 t2 ->
    exists2 s2',
      s1' == s2' & Trace TS s s'' t'.
  Proof with sauto.
    intros Hls Hperm.
    generalize dependent s.
    ...
    - sauto.
    - inversion_clear Hls.
      destruct (IHHperm s'0 H0) as [s_ Hs_ Hl2].
      exists s_; sauto.
    - replace (a :: b :: l) with ([a; b] ++ l) in Hls by reflexivity.
      ...
      apply (H s s_) in Hs_.
      + destruct Hs_ as [s'' Hs'' Hab].
        exists s''.
      specialize (H s s_). unfold equiv_commute_swap in H.
      apply H in Hs_.
...
exists s'.
      + easy.
      + inversion_clear Hls.
        specialize (IHHperm s'0 H0).
        destruct IHHperm as [s_ Hs_ Hl2].
        rewrite Hs_ in
        constructor 2 with (s' := s'0); try assumption.
        rewrite
...
Section trace_equivalence.
  ...
  Definition sufficient_replacement :=
    forall t s s', In e t ->
              Trace T s s' t ->
              exists t',
                In e' t' /\ Trace T s s' t'.

  Definition sufficient_replacement_p :=
    forall t, e t ->
         exists t',
           In e' t' /\ RestrictedPermutation labels_commute t t'.
...
Section setoid_trace_equiv.
  Context  `{TS : TransitionSystem} (e e' : @TraceEnsemble Label).

  Definition setoid_sufficient_replacement :=
    forall t s s', In e t ->
              Trace TS s s' t ->
              exists t' s'',
                In e' t' /\ s' == s'' /\ Trace TS s s'' t'.

  Definition setoid_sufficient_replacement_p :=
    forall t, e t ->
         exists t',
           In e' t' /\ RestrictedPermutation labels_commute_equiv t t'.
End setoid_trace_equiv.
...
Section trans_prod.
  ...
  Global Program Instance transProdSetoid : @Setoid State :=
    { equiv a b :=
        let (a1, b1) := a in
        let (a2, b2) := b in
        a1 == a2 /\ b1 == b2
    }.
  Next Obligation with sauto.
    intros.
    split.
    - intros x...
    - intros x y...
  ...
  Program Instance transProd : @TransitionSystem State Label TransProdResult :=
...
Section trans_sum.
  ...
  Global Program Instance transSumSetoid : @Setoid State :=
    { equiv a b :=
        let (a1, b1) := a in
        let (a2, b2) := b in
        a1 == a2 /\ b1 == b2
...
Section Σ.
  ...
  Inductive Σ' : Σ -> @Future Σ Label True -> Prop :=
  | Σ'₀ : forall s l r σ,
      future s ~>[r] ->
      Σ' (Σ₁ s l σ) ~[l]~> σ
  | Σ'₁ : forall s s' l l' σ,
  ...
  (* Global Instance tsΣ : TransitionSystem Σ Label True := *)
  (*   {future := Σfuture}. *)

  (* Lemma Σø' t σ : Trace tsΣ Σø σ t -> *)
  (*                 t = [] /\ σ = Σø. *)
  (* Proof. *)
  (*   intros H. *)
  (*   inversion H. *)
  ...
  (* Lemma Σ₁' s l σ σ' : Trace tsΣ (Σ₁ s l σ) σ' [] -> *)
  (*                      σ' = (Σ₁ s l σ). *)
  (* Proof. *)
  (*   intros H. *)
  (*   inversion H. *)
...
Hint Unfold future : slot.
Hint Unfold Σfuture : slot.
